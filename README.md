...
- [x] **M5**：账户分 现金/USDT-TRC20/银行；USDT 钱包链上监控（余额+TX 对账，Tronscan API）
- [ ] M6（可选）：Telegram 内联按钮审批 / 双因素登录 / 多币种实时汇率 / USDT 自动入账
...
支持三种付款账户：**💵 现金 / 🔶 USDT-TRC20 / 🏦 银行**。

USDT 强制约束：
- USDT 付款申请必须填收款地址（TRC20）
- 财务付 USDT 时必须填链上交易哈希 `tx_hash`
- 备用金 USDT 支出/归还必须填 `tx_hash`
- 所有 `tx_hash` 自动生成 Tronscan 链接，可一键查看链上记录

USDT 监控页 `/usdt`（财务/管理/admin 可见）：
- 列出所有配置 USDT 地址的员工
- **链上余额**（实时从 Tronscan API 查）vs **系统账面余额**，自动算差额
- 单人详情页：链上最近 50 笔 TX + 系统流水，做**自动对账**
  - 链上有但系统未入账 → 标"未入账"
  - 系统有但链上找不到 → 红色异常警告
- 链上 API 不可达时优雅降级（业务不受影响，只是不显示链上数据）

可在后台配置：
- `tron_api_endpoint`（默认 Tronscan 公共 API）
- `tron_api_key`（可选，提速）
- `usdt_trc20_contract`（默认官方合约地址）
